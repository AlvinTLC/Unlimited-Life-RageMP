    ...
    class PublicGarage : Script
    {
                ...
                if (c.Position.DistanceTo2D(garagePosition) < 5)
                {
                    ...
                    if (c.IsInVehicle)
                    {
                        if (c.Vehicle.GetData<Player>("OWNER") == c)
                        {
                            ...
                            {
                                Notify.Send(c, NotifyType.Error, NotifyPosition.MapUp,
                                    $"Sie haben ein Haus, parken Sie in Ihrer Garage!",
                                3000);
                            }
                        }
                        else
        ...
        public static void SpawnVehicle(Player c, string vNumber)
        {
            try
            {
                ...
                {
                    Notify.Send(c, NotifyType.Error, NotifyPosition.JobSms,
                        $"Das Auto ist nicht in der Garage!",
                    3000);
                    return;
                }


                var house = ULife.Houses.HouseManager.GetHouse(c, true);
                if (house == null || house.GarageID == 0)
                {
                    ...
                    /*if (!VehicleManager.getAllPlayerVehicles(c.Name).Contains(vNumber))
                    {
                        ...
                        Notify.Send(c, NotifyType.Warning, NotifyPosition.JobSms,
                    ...
                    if (!access)
                    {
                        Notify.Send(c, NotifyType.Info, NotifyPosition.JobSms,
                            $"Ich glaube, Sie haben Ihre Autoschlüssel verloren. Holen Sie sich einen neuen!",
                        3000);
                ...
                else
                {
                    ...
                    var garage = ULife.Houses.GarageManager.Garages[house.GarageID];
                    ...
                    {
                        Notify.Send(c, NotifyType.Error, NotifyPosition.MapUp,
                            $"Das Auto steht nicht in der offenen Garage!",
                        3000);
                        return;
                    }
                }
            }
            catch (Exception e) { Log.Write("Fehler beim Anlegen einer Maschine aus einer öffentlichen Garage " + e.Message); }
        ...
        public void OnResourceStart()
        {
            ...
            foreach (Vector3 position in vehstore)
            {
                ...
                {
                    NAPI.Blip.CreateBlip(50, position, 0.50f, 4, "Öffentliche Garage", 255, 0, true);
                }
                ...
                shape.OnEntityEnterColShape += (s, entity) =>
                {
                    try
                    {
                        Notify.Send(entity, NotifyType.Info, NotifyPosition.JobSms,
                            $"Drücke Numpad 2, um das Menü zu öffnen!",
                        3000);
                    }
