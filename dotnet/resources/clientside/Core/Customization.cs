...
namespace ULife.Core
{
    ...
    class Customization : Script
    {
        ...
        //public static Vector3 CreatorCharPos = new Vector3(402.90966796875, -999.177001953125, -98.00404357910156);
        //public static Vector3 CreatorPos = new Vector3(402.9683837890625, -996.2813720703125, -99.00025939941406);
        //public static Vector3 CameraLookAtPos = new Vector3(402.90966796875, -999.177001953125, -98.00404357910156);
        ...
        public static void SendBackToWorld(Player player)
        {
            ...
            Vector3 pos = new Vector3(-1136.237, -2743.4998, 21.320065);
        ...
        public static void SetMask(Player player, int variation, int texture)
        {
            if (variation == 0)
            {
                player.SetSharedData("IS_MASK", false); /*Добавил Shared*/
                ApplyCharacterFace(player);
            ...
            {
                player.SetSharedData("IS_MASK", true); /*Добавил Shared*/
        ...
        public void ClientEvent_saveCharacter(Player player, params object[] args)
        {
            try
            {
                ...
                if (genderChanged)
                {
                    ...
                    switch (Main.Players[player].Gender)
                    {
                        case true: // Мужик
                            ...
                            AddClothes(player, ItemType.Top, 146, color, true);
                            CustomPlayerData[Main.Players[player].UUID].Clothes.Top = new ComponentItem(146, color);
                            color = Main.rnd.Next(0, 6);
                            AddClothes(player, ItemType.Leg, 102, color, true);
                            CustomPlayerData[Main.Players[player].UUID].Clothes.Leg = new ComponentItem(102, color);
                        ...
                        case false: // Женщина
                            color = Main.rnd.Next(0, 6);
                            AddClothes(player, ItemType.Top, 30, color, true);
                            CustomPlayerData[Main.Players[player].UUID].Clothes.Top = new ComponentItem(30, color);
                            color = Main.rnd.Next(0, 16);
                            AddClothes(player, ItemType.Leg, 43, color, true);
                            CustomPlayerData[Main.Players[player].UUID].Clothes.Leg = new ComponentItem(43, color);
                            color = Main.rnd.Next(0, 16);
                            AddClothes(player, ItemType.Feet, 3, color, true);
                            CustomPlayerData[Main.Players[player].UUID].Clothes.Feet = new ComponentItem(3, color);
                            break;
                    }
                }
                ...
                if (!isChanging)
                {
                    ...
                    if (!Main.Accounts[player].PresentGet && Main.Accounts[player].PromoCodes[0].Equals("ulife"))
                    {
                        Main.Accounts[player].PresentGet = true;
                        GameLog.Money($"server", $"player({Main.Players[player].UUID})", 10000, $"ulife");
                        nInventory.Add(player, new nItem(ItemType.Kaffee, 3));
                        nInventory.Add(player, new nItem(ItemType.Donut, 3));
                        Main.Players[player].LVL = 1;
                        MoneySystem.Wallet.Change(player, 10000);
                        Main.Accounts[player].VipLvl = 4;
                        Main.Accounts[player].VipDate = DateTime.Now.AddDays(10);
                        GUI.Dashboard.sendStats(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.JobSms, "Herzlich willkommen! Sie haben Ihr erstes Level, Platinum VIP für 10 Tage und 10000$ erhalten!", 6000);
