...
namespace ULife.Core
{
    ...
    class Ban : BanData
    {
        ...
        #region Alle Arten von Inspektionen
        // Prüfung auf Übereinstimmung von HWID und IP-Adressen
        public static Ban Get1(Player Player)
        {
            lock (Banned)
            {
                ...
                {
                    ban = Banned.FindLast(x => x.SocialClub == Player.GetData<string>("RealSocialClub"));
                    if (ban != null) return ban;
                }
                ban = Banned.FindLast(x => x.IP == Player.Address);
                if (ban != null) return ban;
                if (Player.HasData("RealHWID")) ban = Banned.FindLast(x => x.HWID == Player.GetData<string>("RealHWID"));
                return ban;
            }
        }

        // Suche nach Zeichen UUID
        ...
        #region Erteilen eines Verbots
        public static void Online(Player Player, DateTime until, bool ishard, string reason, string admin)
        {
            var acc = Main.Players[Player];
            if (acc == null)
            {
                Log.Write($"Can't ban player {Player.Name}", nLog.Type.Error);
                return;
            }
            Ban ban = new Ban()
            {
                ...
                Account = Main.Accounts[Player].Login,
                Time = DateTime.Now,
                Until = until,
                isHard = ishard,
                IP = Player.Address,
                SocialClub = Player.GetData<string>("RealSocialClub"),
        ...
        #region Nehmen Sie den Hardban ab
