...
SELECT * FROM questions;";
        ...
        public void ReportTake(Player Player, int id, bool retrn = false)
        {
            ...
            foreach (Player target in NAPI.Pools.GetAllPlayers())
            {
                ...
                else Trigger.ClientEvent(target, "setreport", id, Player.Name);
            }
        }
        ...
        public void ReportSend(Player player, int ID, string answer)
        {
            ...
            {
                player.SendChatMessage("Diese Reklamation ist nicht mehr zur Änderung verfügbar.");
                Remove(ID, player);
            }
        }
        #endregion
        ...
        public static void AddReport(Player player, string question)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Der Bericht wurde gesendet", 3000);
        ...
        private static void AddAnswer(Player player, int repID, string response)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (target is null)
                    {
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.MapUp, "Kein Spieler gefunden!", 3000);
                    }
                    else
                    {
                        target.SendChatMessage($"~r~Antwort von {player.Name} ({player.Value}): {response}");
                        Notify.Send(target, NotifyType.Info, NotifyPosition.MapUp, $"Sie haben eine Antwort auf den Bericht erhalten", 5000);
                        foreach (var p in Main.Players.Keys.ToList())
                        {
                            if (Main.Players[p].AdminLVL >= adminLvL)
                            {
                                p.SendChatMessage($"~y~[ОТВЕТ] {player.Name}({player.Value})->{target.Name}({target.Value}): {response}");
                            }
                        }
