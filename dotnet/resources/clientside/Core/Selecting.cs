    ...
    class Selecting : Script
    {
                    ...
                    case "ClothesSafe":
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Nur der Hauseigentümer kann die Möbel benutzen.", 3000);
                                return;
                            }
        ...
        public static void vehicleSelected(Player player, params object[] arguments)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Das Auto ist weit weg von Ihnen", 3000);
                    return;
                }
                switch (index)
                {
                    case 0:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie können die Motorhaube nicht öffnen/schließen, während Sie sich im Fahrzeug befinden", 3000);
                            return;
                        }
                        if (VehicleStreaming.GetDoorState(vehicle, DoorID.DoorHood) == DoorState.DoorClosed)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie können die Motorhaube nicht öffnen, wenn das Fahrzeug geschlossen ist", 3000);
                                return;
                            }
                    ...
                    case 1:
                        if (player.IsInVehicle)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie können den Kofferraum nicht vom Fahrzeuginneren aus öffnen/schließen", 3000);
                            return;
                        }
                        ...
                        else
                        {
                            if (vehicle.HasData("ACCESS") && (vehicle.GetData<string>("ACCESS") == "PERSONAL" || vehicle.GetData<string>("ACCESS") == "GARAGE"))
                            {
                                ...
                                if (!access)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Нет ключей", 3000);
                                    return;
                                }
                            }
                            if (vehicle.HasData("ACCESS") && vehicle.GetData<string>("ACCESS") == "FRACTION" && vehicle.GetData<int>("FRACTION") != Main.Players[player].FractionID)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie können den Kofferraum dieses Autos nicht öffnen", 3000);
                                    return;
                                }
                            }
                    ...
                    case 3:
                        if (player.IsInVehicle)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie können das Inventar nicht öffnen, während Sie sich im Fahrzeug befinden", 3000);
                            return;
                        }
                        if (NAPI.Data.GetEntityData(vehicle, "ACCESS") == "WORK" || vehicle.Class == 13 || vehicle.Class == 8)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Dieses Fahrzeug unterstützt keine Lagerhaltung", 3000);
                            return;
                        }
                        if (Main.Players[player].AdminLVL == 0 && VehicleStreaming.GetDoorState(vehicle, DoorID.DoorTrunk) == DoorState.DoorClosed)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie können das Inventar des Geräts nicht öffnen, wenn der Kofferraum geschlossen ist", 3000);
                            return;
                        }
                        if (vehicle.GetData<bool>("BAGINUSE") == true)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Warten Sie, bis die andere Person aufgehört hat, den Kofferraum zu benutzen.", 3000);
                            return;
                        }
                        vehicle.SetData("BAGINUSE", true);
                        GUI.Dashboard.OpenOut(player, vehicle.GetData<List<nItem>>("ITEMS"), "Kofferraum", 2);
        ...
        public static void playerSelected(Player player, params object[] arguments)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Der Spieler ist weit von Ihnen entfernt", 3000);
                    return;
                }
                ...
                switch (action)
                {
                    ...
                    case "Los lassen":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Niemand schleppt diesen Spieler", 3000);
                            return;
                        }
                        if (!player.HasData("FOLLOWER") || player.GetData<Player>("FOLLOWER") != target)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Dieser Spieler wird von jemand anderem gezogen", 3000);
                            return;
                        }
                    ...
                    case "Reisepass vorzeigen":
                        if (player.IsInVehicle) return;
                        {
                            ...
                            string gender = (acc.Gender) ? "Männlich" : "Weiblich";
                    ...
                    case "Waffe abnehmen":
                        ...
                        return;
                    case "Illegale Sachen abnehmen":
                        if (player.IsInVehicle) return;
                        playerTakeIlleagal(player, target);
                        return;
                    case "Erste-Hilfe-Kit verkaufen":
                        Trigger.ClientEvent(player, "openInput", "Verkaufen Sie den Erste-Hilfe-Kasten", "Preis $$$", 4, "player_medkit");
                        return;
                    case "Heilung anbieten":
                        if (player.IsInVehicle) return;
                        Trigger.ClientEvent(player, "openInput", "Behandlung anbieten", "Preis $$$", 4, "player_heal");
                        return;
                    case "Heilen":
                        if (player.IsInVehicle) return;
                        playerHealTarget(player, target);
                        return;
                    case "Fahrzeug verkaufen":
                    ...
                    case "Haus verkaufen":
                        ...
                        Trigger.ClientEvent(player, "openInput", "Das Haus verkaufen", "Preis $$$", 8, "player_offerhousesell");
                        return;
                    case "in WG einladen":
                        HouseManager.InviteToRoom(player, target);
                        return;
                    case "in mein Haus einladen":
                        HouseManager.InvitePlayerToHouse(player, target);
                        return;
                    case "Geld geben":
                        if (Main.Players[player].LVL < 1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Geldtransfer ist nach der ersten Stufe verfügbar", 3000);
                            return;
                        }
                        Trigger.ClientEvent(player, "openInput", "Übergeben Sie das Geld", "Preis $$$", 4, "player_givemoney");
                        return;
                    case "Handeln":
                        target.SetData("OFFER_MAKER", player);
                        target.SetData("REQUEST", "OFFER_ITEMS");
                        target.SetData("IS_REQUESTED", true);
                        Notify.Send(target, NotifyType.Warning, NotifyPosition.MapUp, $"Игрок ({player.Value}) angeboten, Gegenstände zu tauschen. Y/N - akzeptieren/ablehnen", 3000);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Sie haben einen Spieler angeboten ({target.Value}) Tauschartikel.", 3000);
                        return;
                    case "Tasche auf- / abnehmen":
                        if (player.IsInVehicle) return;
                    ...
                    case "Maske abnehmen":
                        ...
                        return;
                    case "Ticket ausstellen":
                        if (player.IsInVehicle) return;
                        player.SetData("TICKETTARGET", target);
                        Trigger.ClientEvent(player, "openInput", "Geldstrafe ausstellen (Betrag)", "Betrag von 0$ bis 7000$", 4, "player_ticketsum");
                        return;
                }
            }
        ...
        public static void playerTransferMoney(Player player, string arg)
        {
            if (Main.Players[player].LVL < 1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Der Geldtransfer wird ab Stufe 1 verfügbar sein.", 3000);
                return;
            }
            try
            {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Sie haben nicht genügend Geldmittel", 3000);
                return;
            }
            if (player.HasData("NEXT_TRANSFERM") && DateTime.Now < player.GetData<DateTime>("NEXT_TRANSFERM") && Main.Players[player].AdminLVL == 0)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Seit der letzten Geldüberweisung ist nicht viel Zeit vergangen.", 3000);
                return;
            }
            player.SetData("NEXT_TRANSFERM", DateTime.Now.AddMinutes(1));
            Notify.Send(target, NotifyType.Info, NotifyPosition.MapUp, $"Spieler ({player.Value}) hat es an Sie weitergegeben. {amount}$", 3000);
            Notify.Send(player, NotifyType.Success, NotifyPosition.MapUp, $"Sie haben einen Spieler übergeben ({target.Value}) {amount}$", 3000);
            ...
            Commands.RPChat("me", player, $"übermittelt durch(а) {amount}$ " + "{name}", target);
        }
        public static void playerHealTarget(Player player, Player target)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Sie haben keinen Erste-Hilfe-Kasten", 3000);
                    return;
                }
                ...
                if (target.HasData("IS_DYING"))
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Sie haben mit der Wiederbelebung eines Spielers begonnen ({target.Value})", 3000);
                    Notify.Send(target, NotifyType.Info, NotifyPosition.MapUp, $"Spieler ({player.Value}) begonnen, Sie wiederzubeleben", 3000);
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                            ...
                            if (Main.Players[player].FractionID != 8)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Wiederbelebung fehlgeschlagen.", 3000);
                                    return;
                                }
                            }
                            else
                ...
                {
                    Notify.Send(target, NotifyType.Info, NotifyPosition.MapUp, $"Spieler ({player.Value}) Sie mit dem Erste-Hilfe-Kasten geheilt", 3000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.MapUp, $"Sie haben einen Spieler geheilt ({target.Value}) mit Hilfe des Erste-Hilfe-Kastens", 3000);
                    target.Health = 100;
                }
                return;
            }
        ...
        public static void playerTakeGuns(Player player, Player target)
        {
            ...
            Notify.Send(target, NotifyType.Warning, NotifyPosition.MapUp, $"Spieler ({player.Value}) alle Ihre Waffen beschlagnahmt", 3000);
            Notify.Send(player, NotifyType.Success, NotifyPosition.MapUp, $"Sie haben alle Waffen von einem Spieler genommen ({target.Value})", 3000);
            return;
        }
        public static void playerTakeIlleagal(Player player, Player target)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Der Spieler hat nichts Verbotenes", 3000);
                return;
            }
            nInventory.Remove(target, ItemType.Material, materials);
            nInventory.Remove(target, ItemType.Drugs, drugs);
            Notify.Send(target, NotifyType.Warning, NotifyPosition.MapUp, $"Spieler ({player.Value}) beschlagnahmte illegale Gegenstände von Ihnen", 3000);
            Notify.Send(player, NotifyType.Success, NotifyPosition.MapUp, $"Sie nahmen von einem Spieler {target.Value} verbotene Gegenstände", 3000);
            return;
        }
        ...
        public static void playerHandshakeTarget(Player player, Player target)
        {
            if ((!player.HasData("CUFFED") && !player.HasSharedData("InDeath")) || player.HasData("CUFFED") && player.GetData<bool>("CUFFED") == false && player.HasSharedData("InDeath") && player.GetSharedData<bool>("InDeath") == false)
            {
                if ((!target.HasData("CUFFED") && !target.HasSharedData("InDeath")) || target.HasData("CUFFED") && target.GetData<bool>("CUFFED") == false && target.HasSharedData("InDeath") && target.GetSharedData<bool>("InDeath") == false)
                {
                    ...
                    target.SetData("IS_REQUESTED", true);
                    Notify.Send(target, NotifyType.Warning, NotifyPosition.MapUp, $"Spieler ({player.Value}) möchte Ihnen die Hand schütteln. Y/N - akzeptieren/ablehnen", 3000);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Sie haben einen Spieler angeboten ({target.Value}) Hände schütteln.", 3000);
                }
                else Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie können einem Spieler im Moment nicht die Hand geben", 3000);
            }
            else Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie können einem Spieler im Moment nicht die Hand geben", 3000);
        }
