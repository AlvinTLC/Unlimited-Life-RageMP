...
namespace ULife.Core
{
    class Weapons : Script
    {
        ...
        public static Dictionary<ItemType, ItemType> WeaponsAmmoTypes = new Dictionary<ItemType, ItemType>()
        {
            ...
            { ItemType.Grenade, ItemType.Grenade },
            { ItemType.BZGas, ItemType.BZGas},
            { ItemType.Molotov, ItemType.Molotov},
            { ItemType.Snowball, ItemType.Snowball},
            { ItemType.Ball, ItemType.Ball},
            { ItemType.SmokeGrenade, ItemType.SmokeGrenade},
            { ItemType.PipeBomb, ItemType.PipeBomb},
            { ItemType.StickyBomb, ItemType.StickyBomb},
        };
        ...
        public static void RemoteEvent_changeWeapon(Player player, int key)
        {
            try
            {
                ...
                switch (key)
                {
                    ...
                    case 4:
                        {
                            var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.Grenade);
                            if (wItem != null) Items.onUse(player, wItem, -1);
                        }
                        return;
                    case 5:
                        {
                            var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.StickyBomb);
                            if (wItem != null) Items.onUse(player, wItem, -1);
                        }
                        return;
                    case 6:
                        {
                            var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.BZGas);
                            if (wItem != null) Items.onUse(player, wItem, -1);
                        }
                        return;
                    case 7:
                        {
                            var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.Molotov);
                            if (wItem != null) Items.onUse(player, wItem, -1);
                        }
                        return;
                    case 8:
                        {
                            var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.Snowball);
                            if (wItem != null) Items.onUse(player, wItem, -1);
                        }
                        return;
                    case 9:
                        {
                            var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.Ball);
                            if (wItem != null) Items.onUse(player, wItem, -1);
                        }
                        return;
                    case 10:
                        {
                            var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.SmokeGrenade);
                            if (wItem != null) Items.onUse(player, wItem, -1);
                        }
                        return;
                    case 11:
                        {
                            var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.PipeBomb);
                            if (wItem != null) Items.onUse(player, wItem, -1);
                        }
                        return;
                }
