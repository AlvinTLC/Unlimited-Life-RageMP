...
namespace ULife.Core.Character
{
    public class Character : CharacterData
    {
            ...
            try
            {
                NAPI.Task.Run(() =>
                {
                    try
                    {
                        ...
                        player.SetData("IS_MASK", false); //player.SetSharedData("IS_MASK", false);
                        ...
                        if (IsBonused)
                        {
                            Trigger.ClientEvent(player, "updatelastbonus", $"der nächste Bonus ist erst morgen verfügbar."); //todo lastbonus
                        }
                        else
                        {
                            ...
                            var hour = date.Hour;
                            var min = date.Minute;
                            Trigger.ClientEvent(player, "updatelastbonus", $"{hour}Std. {min}Min."); //todo lastbonus
                        }
                        #endregion
                        ...
                        else
                        {
                            try
                            {
                                ...
                                Trigger.ClientEvent(player, "ConnectTeamspeak", true);
                ...
                if (Warns > 0 && DateTime.Now > Unwarn)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.MapUp, $"Eine nach unten. Sie sind mit {Warns}", 3000);
                }
            ...
            try
            {
                ...
                foreach (DataRow Row in result.Rows)
                {
                    ...
                    if (Row["pos"].ToString().Contains("NaN"))
                    {
                        ...
                        if (LVL <= 1) SpawnPos = new Vector3(-1136.237, -2743.4998, 21.320065); // На спавне новичков
                        else SpawnPos = new Vector3(-1136.237, -2743.4998, 21.320065); // У мэрии
                    }
                }
                ...
                Spawn(player);
                Trigger.ClientEvent(player, "client::closeapart");
            }
            catch (Exception e)
            {
        ...
        public async Task<int> Create(Player player, string firstName, string lastName)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Name/Nachname Längenfehler", 3000);
                    return -1;
                }
                if (Main.PlayerNames.ContainsValue($"{firstName}_{lastName}"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Dieser Name ist bereits vergeben.", 3000);
                    return -1;
                }
                ...
                SpawnPos = new Vector3(-1131.175, -2744.1738, 21.317032);
