...
namespace ULife.MoneySystem
{
    class Casino : Script
    {
        ...
        private static byte blipColor = config.TryGet<byte>("BlipColor", 5);
        private static uint blipID = config.TryGet<uint>("BlipID", 681);
        ...
        public static void Interact(Player Player)
        {
            if (!PlayersList.ContainsKey(Player))
            {
                ...
                {
                    Notify.Send(Player, NotifyType.Warning, NotifyPosition.MapUp, $"Для игры необходимо иметь баланс больше {minBalance}$", 4000);
                    return;
                }
