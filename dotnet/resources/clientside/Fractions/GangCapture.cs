...
namespace ULife.Fractions
{
    class GangsCapture : Script
    {
        ...
        public static void CMD_startCapture(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du befindest dich nicht in einer der Regionen", 3000);
                return;
            }
            GangPoint region = gangPoints[player.GetData<int>("GANGPOINT")];
            if (region.GangOwner == Main.Players[player].FractionID)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du kannst das Gebiet nicht angreifen", 3000);
                return;
            }
            if (DateTime.Now.Hour < 13 || DateTime.Now.Hour > 23)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du kannst nur zwischen 13:00 und 23:00 Uhr angreifen.", 3000);
                return;
            }
            if (DateTime.Now < nextCaptDate[Main.Players[player].FractionID])
            {
                ...
                var min = g.Minute;
                var sec = g.Second;
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du kannst deine Erfassung nur starten in{min}:{sec}", 3000);
                return;
            }
            if (DateTime.Now < protectDate[region.GangOwner])
            {
                ...
                var min = g.Minute;
                var sec = g.Second;
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Die einzige Möglichkeit, das Territorium dieser Bande zu übernehmen, ist. {min}:{sec}", 3000);
                return;
            }
            if (Manager.countOfFractionMembers(region.GangOwner) < 3)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Es sind zu wenig der gegen Partei wach!", 3000);
                return;
            }
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Der Landraub ist bereits im Gange.", 3000);
                smbTryCapture = false;
                return;
            }
            ...
            Manager.sendFractionMessage(region.GangOwner, $"Achtung! Wir starten in 15 Minuten! {Manager.getName(attackersFracID)} haben beschlossen, unser Gebiet zu übernehmen");
            Manager.sendFractionMessage(attackersFracID, "Los gehts! Quetschen! In etwa 15 Minuten wird der Feind eintreffen");
        }
        ...
        private static void timerStartCapture(GangPoint region)
        {
            ...
            Manager.sendFractionMessage(region.GangOwner, $"Achtung! Wir werden angegriffen! {Manager.getName(attackersFracID)} haben beschlossen, unser Gebiet zu übernehmen");
            Manager.sendFractionMessage(attackersFracID, "los gehts Quetschen! Sie haben einen Revierkampf begonnen");
        }
        ...
        private static void endCapture(GangPoint region, int defenders, int attackers)
        {
            ...
            {
                Manager.sendFractionMessage(region.GangOwner, $"Die Arschlöcher sind entkommen! Du hast ihnen die Hölle heiß gemacht! Sie haben das Territorium verteidigt");
                Manager.sendFractionMessage(attackersFracID, "Sie sind am Arsch! Der Feind war stärker! Sie konnten das Gebiet nicht übernehmen");
            ...
            {
                Manager.sendFractionMessage(region.GangOwner, $"Die haben eine Stelle übersehen.");
                Manager.sendFractionMessage(attackersFracID, "Wir haben das Gebiet erfolgreich eingenommen!");
