...
namespace ULife.Fractions
{
    class LSNews : Script
    {
        ...
        {
            new Vector3(-569.1334, -927.6702, -100.71336), // Колшэйп на крыше LSN
            new Vector3(-562.0423, -927.0765, -50.75758), // Колшэйп изнутри интерьера для телепорта наверх
        };
        ...
        public void OnResourceStartHandler()
        {
            try
            {
                ...
                NAPI.Marker.CreateMarker(1, LSNewsCoords[1] - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220)); //Добавлен
            }
            catch (Exception e)
            {
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                case 80:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du Arbeitest nicht bei Weazel News", 3000);
                        return;
                    }
...
SELECT * FROM advertised;";
        ...
        public void AdvertTake(Player Player, int id, bool retrn = false)
        {
            try
            {
                ...
                foreach (Player target in NAPI.Pools.GetAllPlayers())
                {
                    ...
                    else Trigger.ClientEvent(target, "setadvert", id, Player.Name);
                }
            }
            catch
            {
        ...
        public void AdvertSend(Player player, int ID, string answer)
        {
            try
            {
                ...
                {
                    player.SendChatMessage("Diese Ansage ist nicht mehr zur Änderung verfügbar.");
                    Remove(ID, player);
                }
            }
            catch
        ...
        public static void AddAdvert(Player player, string question, int price)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.MapUp, "Du hast eine Anzeige aufgegeben. Warten auf Moderation", 3000);
                ...
                foreach (Player p in Main.Players.Keys.ToList())
                {
                    ...
                    if (Main.Players[p].AdminLVL >= 1) p.SendChatMessage($"~r~Ankündigung von {player.Name.Replace('_', ' ')} (ID {player.Value} | Ankündigung №{id}): {question}");
                }
            }
            catch (Exception e)
            {
        ...
        public static void AddAnswer(Player player, int repID, string response, bool deleted = false)
        {
            try
            {
                ...
                {
                    if (deleted) player.SendChatMessage("Es wurde keine Anzeige mit einer ähnlichen Nummer gefunden.");
                    return;
                }
                ...
                if (!deleted)
                {
                    try
                    {
                        ...
                        if (Adverts[repID].AuthorSIM >= 1) NAPI.Chat.SendChatMessageToAll("!{#00BCD4}" + $"Ankündigung von {Adverts[repID].Author.Replace('_', ' ')}: {response} | Telefonnummer: {Adverts[repID].AuthorSIM}");
                        else NAPI.Chat.SendChatMessageToAll("!{#00BCD4}" + $"Ankündigung von {Adverts[repID].Author.Replace('_', ' ')}: {response}");
                        NAPI.Chat.SendChatMessageToAll("!{#00BCD4}" + $"Durchsage-Editor: {player.Name.Replace('_', ' ')}.");
                    }
                    catch
                    {
                    }
                ...
                else
                {
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Sie haben die Anzeige eines Spielers gelöscht {Adverts[repID].Author}", 3000);
                    Player target = NAPI.Player.GetPlayerFromName(Adverts[repID].Author);
                    if (target != null) Notify.Send(target, NotifyType.Error, NotifyPosition.MapUp, $"{player.Name} löschte Ihre Anzeige aus folgendem Grund: {response}.", 3000);
                    response += " | Gelöscht";
                }
