...
namespace ULife.Fractions
{
    class Fbi : Script
    {
        ...
        public static Vector3 EnterFBI = new Vector3(105.675, -744.6737, -44.63474); //Вход в FIB телепорт
        private static Vector3 ExitFBI = new Vector3(107.1693, -744.6852, -44.75476); //Выход в FIB телепорт
        private static List<Vector3> fbiCheckpoints = new List<Vector3>()
        {
            new Vector3(147.2835, -757.7181, 241.032), // Смена одежды NPC Steve Hain (0)
            ...
            new Vector3(118.9617, -729.1614, 241.152), // Оружейка, рядом стоит NPC Michael Bisping (5)
            new Vector3(136.0578, -761.8408, 44.75204), // Лифт с 1го этажа (6)
            new Vector3(134.416, -697.5543, 32.20507), // Выход с гаража (7)
            new Vector3(120.0081, -726.7838, 241.032),  // Смена одежды на FIB (8)
            new Vector3(151.8786, -736.7075, 241.032), // Склад FIB (9)
        };
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                case 20:
                    ...
                    else Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du bist kein FIB-Agent", 3000);
                    return;
                case 21:
                ...
                case 24:
                    if (Main.Players[player].FractionID != 9)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du bist kein FIB-Agent", 3000);
                        return;
                    }
                ...
                case 46:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du musst im Dienst sein", 3000);
                        return;
                    }
                    ...
                    else
                    {
                        if (!warg_mode)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Die Stufe Rot ist nicht Aktive", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du hast eine spezielle Uniform angezogen", 3000);
                    }
                    return;
                case 61:
                    if (Main.Players[player].FractionID != 9)
                    ...
                    GUI.Dashboard.OpenOut(player, Stocks.fracStocks[9].Weapons, "Die Waffenkammer", 6);
                    return;
            }
        }
        ...
        public static void callback_fbilift(Player client, int floor)
        {
            try
            {
                ...
                {
                    Notify.Send(client, NotifyType.Error, NotifyPosition.MapUp, $"Du folgst jemanden", 3000);
                    return;
                }
                switch (floor)
                {
            ...
            try
            {
                switch (index)
                {
                    ...
                    case 6: // medkit
                        ...
                        {
                            Notify.Send(client, NotifyType.Error, NotifyPosition.MapUp, "Es gibt keine Erste-Hilfe-Kästen mehr im Lager", 3000);
                            return;
                        }
                    ...
                    case 11:
                        ...
                        Notify.Send(client, NotifyType.Success, NotifyPosition.MapUp, $"Du hast eine FIB-Plakette erhalten", 3000);
                        return;
                }
            }
