...
namespace ULife.Fractions
{
    class CarDelivery : Script
    {
        ...
        {
            { VehicleHash.Speedo4 },
            { VehicleHash.Rumpo2 }, //Перевозка денег
            { VehicleHash.Youga2 },
        };
        ...
        {
            new Vector3(-2165.107, 4285.171, 49.03421), // Шосе Грейт-Оушен Дальнобой колшейп
            new Vector3(142.6606, 6652.569, 31.59082), // ПалеттоБей АЗС колшейп
            new Vector3(1427.403, 6327.671, 24.14281), // Шосе Грейт-Оушен ограбление колшейп
            new Vector3(1421.056, 6355.05, 24.06137), // Шосе Грейт-Оушен ограбление колшейп 2
            new Vector3(2547.709, 4645.736, 34.15304), // Грейпсид-авеню Ферма колшейп
            new Vector3(2529.517, 4109.557, 38.75628), // Ист-Джошуа-роуд колшейп
            new Vector3(1404.127, 3656.653, 34.19704), // Марина-драйв колшейп
            new Vector3(1709.216, 3313.655, 41.2984), // Панорама-драйв колшейп
            new Vector3(1041.522, 2650.708, 39.62605), // Шоссе 68 колшейп
            new Vector3(260.4555, 2580.766, 45.11154), // Сенора-рейд колшейп
        };
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel("", GangStartDelivery + new Vector3(0, 0, 2.5), 5f, 0.4f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension); //~g~Jimmy Lishman
                ...
                foreach (var pos in MafiaStartDelivery)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel("", pos.Value, 5f, 0.4f, 0, new Color(255, 255, 255)); //~g~Get delivery mission
                }
                #endregion
                ...
                colShape.OnEntityEnterColShape += (s, e) =>
                {
                    try
                    {
                        ...
                        Notify.Send(e, NotifyType.Success, NotifyPosition.MapUp, "Вы арестовали машину", 3000);
                    }
                ...
                foreach (var pos in GangEndDelivery)
                {
                    ...
                    colShape.SetData("ID", i); //Shared добавил
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        try
                        {
                            ...
                            {
                                Notify.Send(e, NotifyType.Success, NotifyPosition.MapUp, "Versuchen Sie es später noch einmal", 3000);
                                return;
                            }
                            NAPI.Data.ResetEntityData(e.Vehicle.GetSharedData<Entity>("WHOS_VEH"), "DELIVERY_CAR"); //Добавил*/
                            /*NAPI.Data.ResetEntityData(e.Vehicle.GetSharedData<Vehicle>("WHOS_VEH"), "DELIVERY_CAR");*/
                            e.Vehicle.Delete();
                            Stocks.fracStocks[Main.Players[e].FractionID].Money += 500;
                            GameLog.Money($"server", $"frac({Main.Players[e].FractionID})", 500, "dropCar");
                            e.SendChatMessage("Die Übergabe des Autos: !{#00FF00} $500~w~wurden an den gemeinsamen Fonds der Bande geschickt. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()}" + ")");
                        }
                ...
                foreach (var pos in MafiaEndDelivery)
                {
                    ...
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        try
                        {
                            ...
                            NAPI.Data.ResetEntityData(e.Vehicle.GetSharedData<Entity>("WHOS_VEH"), "DELIVERY_CAR");
                            e.Vehicle.Delete();
                            Stocks.fracStocks[Main.Players[e].FractionID].Money += 500;
                            GameLog.Money($"server", $"frac({Main.Players[e].FractionID})", 500, "dropCar");
                            e.SendChatMessage("Lieferung des Lieferwagens:! {# 00FF00} $ 500 ~ w ~ wurden an den gemeinsamen Mafia-Fonds geschickt. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()})");
                        }
                ...
                switch ((string)vehicle.GetData<string>("ACCESS"))
                {
                    case "GANGDELIVERY":
                        if (fraction == 7 || fraction == 9)
                        {
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, "Bringen Sie das Auto auf die Polizeiwache", 3000);
                        }
                        ...
                        else
                        {
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, "Bringen Sie das Gerät zu dem markierten Punkt auf der Karte.", 3000);
                        }
                        return;
                    case "MAFIADELIVERY":
                        if (fraction == 7 || fraction == 9)
                        ...
                        {
                            if (vehicle.GetData<bool>("DELIVERED"))
                            {
                                var start = (int)vehicle.GetData<int>("STARTPOINT");
                                Trigger.ClientEvent(player, "createWaypoint", MafiaStartDelivery[start].X, MafiaStartDelivery[start].Y);
                                Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, "Fahren Sie mit dem Auto zurück zu dem markierten Punkt auf der Karte", 3000);
                            }
                            else
                            {
                                var end = (int)vehicle.GetData<int>("ENDPOINT");
        ...
        public void Event_gangMission(Player player, int id)
        {
            try
            {
                ...
                {
                    case 0:
                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Zurzeit nicht verfügbar", 3000);
                        return;
                    case 1:
                        if (DateTime.Now < NextDelivery[fraction])
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Versuchen Sie es später erneut", 3000);
                            return;
                        }

                        if (player.HasData("DELIVERY_CAR"))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du musst zuerst das bisherige Gerät ausliefern", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, "Du hast die Koordinaten des Fahrzeugs", 3000);
                        return;
                }
            }
        ...
        public void Event_mafiaMission(Player player, int id)
        {
            try
            {
                ...
                if (player.HasSharedData("DELIVERY_CAR"))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du musst zuerst das bisherige Gerät ausliefern", 3000);
                    return;
                }
                ...
                NextDelivery[fraction] = DateTime.Now.AddMinutes(5);
                Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Fahren Sie den Van zu dem auf der Karte angegebenen Ort", 3000);
                Trigger.ClientEvent(player, "createWaypoint", MafiaEndDelivery[end].X, MafiaEndDelivery[end].Y);
