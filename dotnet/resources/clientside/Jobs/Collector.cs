...
namespace ULife.Jobs
{
    class Collector : Script
    {
        ...
        private static Vector3 TakeMoneyPos = new Vector3(-1537.1552, -577.9207, 24.587807);
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~b~Drücke E, um das Geld einzusammeln"), TakeMoneyPos + new Vector3(0, 0, 0.3), 30f, 0.4f, 0, new Color(255, 255, 255), true, 0);
            }
            ...
            try
            {
                ...
                if (Main.Players[player].WorkID == 7)
                {
                    ...
                    if (!NAPI.Data.GetEntityData(vehicle, "ON_WORK"))
                    {
                        ...
                        {
                            if (Main.Players[player].Money >= 100) Trigger.ClientEvent(player, "openDialog", "COLLECTOR_RENT", "Willst du wirklich im Inkassobüro arbeiten, und ein Fahrzeug für 100$ mieten?");
                            else
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du hast nicht genug " + (100 - Main.Players[player].Money) + "$ für Busvermietung", 3000);
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Das Auto ist besetzt", 3000);
                            VehicleManager.WarpPlayerOutOfVehicle(player);
                        }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du arbeitest nicht als Collector, gehe zum Arbeitsamt um einen Job zu bekommen", 3000);
                    VehicleManager.WarpPlayerOutOfVehicle(player);
                }
            }
        ...
        public void onPlayerExitVehicleHandler(Player player, Vehicle vehicle)
        {
            try
            {
                ...
                NAPI.Data.GetEntityData(player, "WORK") == vehicle)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.MapUp, $"Wenn du nicht in 3 Minuten, in deinem Fahrzeug sitzt ist dein Arbeitstag vorbei", 3000);
        ...
        public static void rentCar(Player player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Du hast deinen Job als Geldeintreiber begonnen. Verteile das Geld an Geldautomaten.", 3000);
        ...
        public static void CollectorTakeMoney(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du hast noch Taschen voller Geld übrig ({player.GetData<int>("COLLECTOR_BAGS")}Stück)", 3000);
                return;
            }
            else
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Du hast die neuen Geld Kasetten bekommen", 3000);
                player.SetData("COLLECTOR_BAGS", 15);
        ...
        public static void CollectorEnterATM(Player player, ColShape shape)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Der Geldautomat ist noch voller Geld, versuche es Später erneut", 3000);
                    return;
                }
                ...
                if (player.GetData<int>("COLLECTOR_BAGS") == 0)
                {
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.MapUp, "Fahre zurück, um neue Kasetten zu besorgen", 3000);
                    Trigger.ClientEvent(player, "deleteWorkBlip");
                ...
                else
                {
                    ...
                    Trigger.ClientEvent(player, "createWaypoint", MoneySystem.ATM.ATMs[x].X, MoneySystem.ATM.ATMs[x].Y);
                    Trigger.ClientEvent(player, "createWorkBlip", MoneySystem.ATM.ATMs[x]);
                    Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Fahre zum nächsten Automaten", 3000);
                }
            }
            catch { }
        }
