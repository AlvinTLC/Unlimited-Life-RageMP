...
namespace ULife.Jobs
{
    class Pizza : Script
    {
        ...
        private static int checkpointPayment = 9;

        public static List<Vector3> Coords = new List<Vector3>()
        {
            new Vector3(136.69688, 94.82483, 82.38765), // start work
            new Vector3(64.48331, 123.934135, 78.03842), // get car
        };
        ...
        public static void onPlayerDisconnected(Player player, DisconnectionType type, string reason)
        {
            try
            {
                ...
                if (player.GetData<Vehicle>("WORK") != null) NAPI.Entity.DeleteEntity(player.GetData<Vehicle>("WORK"));
            }
        ...
        public static void GoPostal_onEntityEnterColShape(ColShape shape, Player player)
        {
            try
            {
                ...
                if (player.HasData("NEXTHOUSE") && player.HasData("HOUSEID") && NAPI.Data.GetEntityData(player, "NEXTHOUSE") == player.GetData<int>("HOUSEID"))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Verlasse das Fahrzeug", 3000);
                        return;
                    }
                    ...
                    else if (player.GetData<int>("PACKAGES") > 1)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Du hast {player.GetData<int>("PACKAGES")} Pakete", 3000);
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Der Besitzer ist nicht zu Hause, versuche es später noch einmal", 3000);
                            return;
                        }
                    ...
                    else
                    {
                        ...
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.MapUp, $"Wenn du keine Pizzen mehr hast, fahre zurück und besorge dir neue", 3000);
                    }
                }
            }
        ...
        public static void getGoPostalCar(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du arbeitest nicht als Pizza Lieferant", 3000);
                return;
            }
            if (!player.GetData<bool>("ON_WORK"))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du musst deinen Arbeitstag beginnen", 3000);
                return;
            }
            ...
            var veh = API.Shared.CreateVehicle(VehicleHash.Boxville2, player.Position + new Vector3(0, 0, 1), player.Rotation.Z, 10, 10);
            player.SetSharedData("WORK", veh);
            Notify.Send(player, NotifyType.Info, NotifyPosition.MapUp, $"Du hast ein funktionierendes Fahrzeug bekommen", 3000);
