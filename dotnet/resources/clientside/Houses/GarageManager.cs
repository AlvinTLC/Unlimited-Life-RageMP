...
namespace ULife.Houses
{
    ...
    class Garage
    {
                ...
                NAPI.Task.Run(() => {
                    try
                    {
                        if (VehicleManager.Vehicles.ContainsKey(number))
                        {
                            ...
                            VehicleManager.Vehicles[number].Fuel = (!NAPI.Data.HasEntitySharedData(entityVehicles[number].Item2, "PETROL")) ? VehicleManager.VehicleTank[vclass] : (int)NAPI.Data.GetEntitySharedData(entityVehicles[number].Item2, "PETROL");
                        }
                ...
                NAPI.Task.Run(() =>
                {
                    try
                    {
                        if (VehicleManager.Vehicles.ContainsKey(number))
                        {
                            ...
                            var vclass = NAPI.Vehicle.GetVehicleClass(NAPI.Util.VehicleNameToModel(VehicleManager.Vehicles[number].Model));
                            VehicleManager.Vehicles[number].Fuel = (!NAPI.Data.HasEntitySharedData(vehiclesOut[number], "PETROL")) ? VehicleManager.VehicleTank[vclass] : (int)NAPI.Data.GetEntitySharedData(vehiclesOut[number], "PETROL");
                        }
        ...
        public void SpawnCar(string number)
        {
            ...
            var veh = NAPI.Vehicle.CreateVehicle(NAPI.Util.GetHashKey(vehData.Model), GarageManager.GarageTypes[Type].VehiclesPositions[i] + new Vector3(0, 0, 0.25), GarageManager.GarageTypes[Type].VehiclesRotations[i].Z, 0, 0);
            veh.Rotation = GarageManager.GarageTypes[Type].VehiclesRotations[i];
            ...
            veh.SetSharedData("PETROL", vehData.Fuel); //Shared
        ...
        public void SpawnCarAtPosition(Player player, string number, Vector3 position, Vector3 rotation)
        {
            ...
            veh.SetSharedData("PETROL", vData.Fuel); //Shared
        ...
        {
            var vData = VehicleManager.Vehicles[number];
            var veh = NAPI.Vehicle.CreateVehicle(NAPI.Util.GetHashKey(vData.Model), player.Position + new Vector3(0, 0, 0.3), Rotation.Z, 0, 0, number);
            veh.Rotation = Rotation;
        ...
        public void SendAllVehiclesToGarage()
        {
            try
            {
                ...
                foreach (var v in vehiclesOut)
                {
                    ...
                    VehicleManager.Vehicles[v.Key].Fuel = (!NAPI.Data.HasEntitySharedData(v.Value, "PETROL")) ? VehicleManager.VehicleTank[vclass] : (int)NAPI.Data.GetEntitySharedData(v.Value, "PETROL"); //Shared добавил
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
            ...
            try
            {
                foreach (var v in toSend)
                {
                    if (vehiclesOut.ContainsKey(v))
                    {
                        ...
                        VehicleManager.Vehicles[v].Fuel = (!NAPI.Data.HasEntitySharedData(vehiclesOut[v], "PETROL")) ? VehicleManager.VehicleTank[vclass] : (int)NAPI.Data.GetEntitySharedData(vehiclesOut[v], "PETROL"); //Shared добавил
                        NAPI.Task.Run(() =>
                        {
                            try
                            {
    ...
    class GarageManager : Script
    {
            ...
            try
            {
                switch (id)
                {
                    case 40:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du kannst das Auto nicht in die Garage fahren", 3000);
                            return;
                        }
                        else if (player.IsInVehicle && vehicles.Contains(player.Vehicle.NumberPlate))
                        {
                            ...
                            VehicleManager.Vehicles[number].Fuel = (!NAPI.Data.HasEntitySharedData(player.Vehicle, "PETROL")) ? VehicleManager.VehicleTank[player.Vehicle.Class] : (int)NAPI.Data.GetEntitySharedData(player.Vehicle, "PETROL"); //Shared добавил
                        ...
                        if (garage.Type == -1)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Dein Auto steht irgendwo in der gegend du kannst es abschleppen", 3000);
                                return;
                            }
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du musst dein Auto neu wieder zusammen bauen", 3000);
                                return;
                            }
        ...
        public static void CMD_SetGarage(Player player, int ID)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du musst dich in den Eingangsbereich stellen", 3000);
                return;
            }
        ...
        public static void CMD_RemoveGarage(Player player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du solltest dich vor die Garage stellen", 3000);
                return;
            }
