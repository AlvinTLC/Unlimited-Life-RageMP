                    ...
                    case 0:
                        {// self inventory
                            ...
                            {//remove one item from player inventory
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du kannst im moment den Kanister nicht wegwerfen", 3000);
                                    return;
                                }
                                ...
                                else if (nInventory.ClothesItems.Contains(item.Type))
                                {
                                    if (item.IsActive)
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du musst zuerst deine Kleidung ausziehen", 3000);
                                        return;
                                    }
                                ...
                                else if (nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type))
                                {
                                    if (item.IsActive)
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du musst deine Waffe aus der Hand nehmen", 3000);
                                        return;
                                    }
                                    items.RemoveAt(index);
                                    Items.onDrop(player, new nItem(item.Type, 1, item.Data), null);
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du kannst keine Gegenstände wegwerfen, wärend du im Auto sitzt", 3000);
                                    return;
                                }
                                if (item.Count > 1)
                                {
                                    ...
                                    Trigger.ClientEvent(player, "openInput", "Weg werfen", "anzahl eingeben", 3, "item_drop");
                                    return;
                                }
                            ...
                            else if (data == "use")
                            {
                                try
                                {
                                    ...
                                    {
                                        Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Um Gegenstände zu benutzen, musst den Gegenstand erst entgegen nehmen", 3000);
                                        return;
                                    }
                            ...
                            else if (data == "transfer")
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du musst zuerst die Kleidung ausziehen", 3000);
                                    return;
                                }
                                else if ((nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type)) && item.IsActive == true)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Du musst zuerst die Waffe aus der Hand nehmen", 3000);
                                    return;
                                }
                                switch (player.GetData<int>("OPENOUT_TYPE"))
                                {
                                    ...
                                    case 2:
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Der Kofferraum hat nicht genug Platz", 3000);
                                                return;
                                            }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openInput", "Gegenstand umladen", "Anzahl eingeben", 3, "item_transfer_toveh");
                                                return;
                                            }
                                            if (item.Type == ItemType.Material)
                                            {
                                                ...
                                                {
                                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du kannst nicht soviel auf einmal umladen", 3000);
                                                    return;
                                                }
                                            }
                                    ...
                                    case 3:
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Dieser Gegenstand passt nicht in den Schrank", 3000);
                                                return;
                                            }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openInput", "Gegenstand umladen", "Anzahl eingeben", 3, "item_transfer_tosafe");
                                                return;
                                            }
                                    ...
                                    case 4:
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Der Schrank ist für Kleidung gedacht", 3000);
                                                return;
                                            }
                                            if (Main.Players[player].InsideHouseID == -1) return;
                                            int houseID = Main.Players[player].InsideHouseID;
                                    ...
                                    case 5:
                                        {
                                            ...
                                            if (item.Type == ItemType.BagWithDrill)
                                            {
                                                ...
                                                {
                                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Dein gegenüber hat schon was in der Hand", 3000);
                                                    return;
                                                }
                                            ...
                                            else if (item.Type == ItemType.BagWithMoney)
                                            {
                                                if (target.HasData("HEIST_DRILL") || target.HasData("HAND_MONEY"))
                                                {
                                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du hast bereits eine Tasche auf den Rücken", 3000);
                                                    return;
                                                }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openInput", "Gegenstand übergeben", "Anzahl eingeben", 3, "item_transfer_toplayer");
                                                return;
                                            }
                                    ...
                                    case 6:
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Das Lager hat nicht genug Platz", 3000);
                                                return;
                                            }
                                            ...
                                            if (item.Count > 1)
                                            {
                                                ...
                                                player.SetData("ITEMTYPE", item.Type);
                                                player.SetData("ITEMINDEX", index);
                                                Trigger.ClientEvent(player, "openInput", "Gegenstand übergeben", "Anzahl eingeben", 3, "item_transfer_tofracstock");
                                                return;
                                            }
                                    ...
                                    case 7:
                                        {
                                            ...
                                            if (keys.Count >= 5)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Maximal 5 Schlüssel!", 3000);
                                                return;
                                            }

                                            if (item.Type != ItemType.CarKey)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Nur für Schlüssel verwendbar", 3000);
                                                return;
                                            }
                                            ...
                                            OpenOut(player, keyringItems, "Ein Schlüsselbund", 7);
                                            return;
                                        }
                                    case 8: // Оружейный сейф
                                        {
                                            ...
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Ein Waffenschrank kann nur Waffen aufbewahren", 3000);
                                                return;
                                            }
                                            if (Main.Players[player].InsideHouseID == -1) return;
                                            int houseID = Main.Players[player].InsideHouseID;
                                    ...
                                    case 9:
                                        {
                                            int ItemBox = player.GetData<int>("ITEMBOX");
                                            nItem keyring;
                                            try
                                            {
                                                keyring = nInventory.Items[Main.Players[player].UUID][ItemBox];
                                            }
                                            catch
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Versuchen Sie erneut, den Rucksack zu öffnen", 3000);
                                                Log.Write(Main.Players[player].UUID + " versuchte nicht existierenden Rucksack zu benutzen");
                                                return;
                                            }

                                            if (keyring.Type != ItemType.ItemBox)
                                            {
                                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Versuchen Sie erneut, den Rucksack zu öffnen (2)", 3000);
                                                Log.Write(Main.Players[player].UUID + " versuchte nicht existierenden Rucksack zu benutzen");
                                                return;
                                            }

                                            // Get all objects from keyring


                                            // Get keyring items
                            //List<nItem> keyRingItems = JsonConvert.DeserializeObject<List<nItem>>(Convert.ToString(keyring.Data));
                            List<nItem> keyRingItems = keyring.Data;

                            // Get seleted item
                            nItem selectedItem = keyRingItems[index];

                            // Try to move selected item into player inventory
                            int tryAdd = nInventory.TryAdd(player, selectedItem);
                            if (tryAdd == -1 || tryAdd > 0)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Sie haben nicht genug Platz", 3000);
                                return;
                            }

                            // Remove selected item from keyring
                            keyRingItems.Remove(selectedItem);

                            // Update keyring Data
                            //keyring.Data = JsonConvert.SerializeObject(keyRingItems);
                            //keyring.Data = keyring.Data.Replace("\"", "\\\"");
                            //Log.Write(keyring.Data);
                            keyring.Data = keyRingItems;

                            // Save keyring
                            nInventory.Items[Main.Players[player].UUID][ItemBox] = keyring;

                            // Update player data
                            player.SetData("ITEMBOX", nInventory.Items[Main.Players[player].UUID].IndexOf(keyring));

                            // Add selected iten into player inventory
                            nInventory.Add(player, selectedItem);

                            // Refresh
                            OpenOut(player, keyRingItems, "Rucksag", 9);
                            break;
                        }
                                    case 10:
                                        {
                                            // Remove item from player inventory
                                            Notify.Send(player, NotifyType.Alert, NotifyPosition.MapUp, $"Item {nInventory.ItemsNames[(int)item.Type]} is destroyed", 3000);
                                            nInventory.Remove(player, item);
                                            return;
                                        }
                    ...
                    case 2:
                        { // in car items
                            ...
                            if (item.Type == ItemType.BagWithDrill)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du hast bereits etwas in der Hand", 3000);
                                    return;
                                }
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openInput", "Gegenstand nehmen", "Anzahl eingeben", 3, "item_transfer_fromveh");
                                return;
                            }
                    ...
                    case 3: // Взять
                        {
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openInput", "Gegenstand nehmen", "Anzahl eingeben", 3, "item_transfer_fromsafe");
                                return;
                            }
                    ...
                    case 6:
                        {
                            ...
                            if (item.Count > 1)
                            {
                                ...
                                player.SetData("ITEMTYPE", item.Type);
                                player.SetData("ITEMINDEX", index);
                                Trigger.ClientEvent(player, "openInput", "Gegenstand nehmen", "Anzahl eingeben", 3, "item_transfer_fromfracstock");
                                return;
                            }
                    ...
                    case 20:
                        {
                            if (Main.Players[player].AdminLVL >= 6 && Main.Players[player].InsideHouseID == -1)
                            {
                                ...
                                if (nInventory.ClothesItems.Contains(item.Type) && item.IsActive == true)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Dein gegenüber muss zuerst die Kleidung ausziehen", 3000);
                                    return;
                                }
                                else if ((nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type)) && item.IsActive == true)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, "Dein gegenüber muss zuerst die Waffe aus der Hand nehmen", 3000);
                                    return;
                                }
                                ...
                                if (item.Count > 1)
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.MapUp, $"Du kannst diese Dinge nicht wegnehmen", 3000);
                                    return;
                                }
                                nInventory.Add(player, item);
