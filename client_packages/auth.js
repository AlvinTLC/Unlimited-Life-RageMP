var cam = mp.cameras.new('default', new mp.Vector3(-95, 19, 1182), new mp.Vector3(0, 0, 0), 70); // Позиция камеры авторизации
cam.pointAtCoord(-95, 19, 0);//угол наклона
cam.setActive(true);
...
mp.events.add('signin', function (authData) {
    if (new Date().getTime() - lastButAuth < 3000) {
        mp.events.call('notify', 4, 9, "Zu schnell", 3000);
        return;
    }
...
mp.events.add('signup', function (regData) {
    ...
    if (checkLgin(username) || username.length > 50) {
        mp.events.call('notify', 1, 9, 'Die Anmeldung hat nicht das richtige Format oder ist zu lang!', 3000);
        return;
    }

    if (pass1 != pass2 || pass1.length < 3) {
        mp.events.call('notify', 1, 9, 'Passwort-Fehler!', 3000);
        return;
    }
...
mp.events.add('newChar', function (slot, name, lastname) {
    if (checkName(name) || !checkName2(name) || name.length > 25 || name.length <= 2) {
        mp.events.call('notify', 1, 9, 'Korrektes Namensformat: 3-25 Zeichen und der erste Buchstabe des Namens wird groß geschrieben', 3000);
        return;
    }

    if (checkName(lastname) || !checkName2(lastname) || lastname.length > 25 || lastname.length <= 2) {
        mp.events.call('notify', 1, 9, 'Korrektes Format des Nachnamens: 3-25 Zeichen und erster Buchstabe des Nachnamens groß geschrieben', 3000);
        return;
    }
...
mp.events.add('transferChar', function (slot, name, lastname, pass) {
    if (checkName(name)) {
        mp.events.call('notify', 1, 9, 'Der Name hat nicht das richtige Format oder ist zu lang!', 3000);
        return;
    }

    if (checkName(lastname)) {
        mp.events.call('notify', 1, 9, 'Nachname hat nicht das richtige Format oder ist zu lang!', 3000);
        return;
    }
