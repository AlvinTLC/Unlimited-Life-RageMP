...
var editing_types = ["Position X", "Position Y", "Höhe", "X Rotation", "Y Rotation", "Drehen"];
...
mp.keys.bind(0x4D, false, function () { // N key change to M
    if (chatActive || !editing) return;
    object.destroy();
    object = null;
    editing = false;
...
mp.events.add('render', () => {
    ...
    AddInstructionalButton("Nächster Modus", 197);
    AddInstructionalButton("Vorheriger Modus", 196);
    AddInstructionalButton("Bauleitung", 194);
    AddInstructionalButton("Bauleitung", 195);
    AddInstructionalButtonCustom("Geschwindigkeit erhöhen", "t_+");
    AddInstructionalButtonCustom("Geschwindigkeit reduzieren", "t_-");
    AddInstructionalButtonCustom("Installiere", "t_Y");
    AddInstructionalButtonCustom("Stornieren", "t_N");
    AddInstructionalEnd(1);

    mp.game.graphics.drawText(`Bearbeitungsmodus: ${editing_types[editing_type_idx]}\nСкорость: ${moving_speeds[moving_speed_idx]}`, [0.5, 0.9], {
        font: 0,
