...
mp.keys.bind(Keys.VK_G, false, function () { // F2 key
...
mp.events.add('render', () => {  // Эта Куета связанна с Колшейпом взаимодействие 
	try {
        if (!loggedin) return;
		if(pedsaying != null) {
	    ...
	    if (nearestObject != null && (entity == null || entity.type != "object")) {
		    mp.game.graphics.drawText("G", [nearestObject.position.x, nearestObject.position.y, nearestObject.position.z], {
			    font: 0,
	            color: [255, 255, 255, 185],
		        scale: [0.4, 0.4],
			    outline: true
			...
			if(truckorderveh == null || entity != truckorderveh) {
				...
				mp.game.graphics.drawText("Dein Auftrag", [entity.position.x, entity.position.y, entity.position.z], {
					font: 1,
