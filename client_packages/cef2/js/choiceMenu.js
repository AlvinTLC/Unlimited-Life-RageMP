$(document).ready(() => {
    ...
    var menus = {
		...
		"accept_job_bus": {
			text: "Вы хотите начать работу <Работа>?",
			on: (values) => {
				menus["accept_job_bus"].text = `Вы хотите ${values.name}`;
			},
			yes: () => {
				mp.trigger("events.callRemote", "job.bus.agree");
			},
			no: () => {
				mp.trigger("client.job.cursor.cancel");
			},
        ...
        "sellcar_confirm": {
            ...
            on: (values) => {
                menus["sellcar_confirm"].text = `Вы действительно хотите продать ${values.model} за ${values.price * 0.30}$-${values.price * 0.40}$?`;
            },
            yes: () => {
        ...
        "accept_fix_car": {
            text: "Вы уверены, что хотите доставить транспорт за 500$?",
            on: (sqlId) => {
                menus["accept_fix_car"].text = `Вы уверены, что хотите доставить транспорт за 500$?`;
    ...
    window.choiceMenuAPI = {
        show: (name, values) => {
			...
			var times = 9;
			$(`.choiceMenu .buttons .button.decline`).attr(`data-close-timer`, `9 с`);
            clearInterval(timerId);
            timerId = setInterval(() => {
                times--;
				$(`.choiceMenu .buttons .button.decline`).attr(`data-close-timer`, `${times} с`);
                if (times < 1) {
                    clearInterval(timerId);
                    choiceMenuAPI.hide();
                }
			...
			$(".choiceMenu .decline").on("click", () => {
                currentChoiceMenu.no();
                choiceMenuAPI.hide();
            });
			$(".choiceMenu .accept").on("click", () => {
        ...
        hide: () => {
			$(".choiceMenu .decline").off("click");
			$(".choiceMenu .accept").off("click");
