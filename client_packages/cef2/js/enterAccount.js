$(document).ready(() => {
	$(".authentication #auth .switch_auth").on("click", () => {
	...
	$("#authenticationApp .authentication input").on("keyup", (e) => {
        if (e.keyCode == 13) { //enter
            authAccountHandler();
        }
    });
...
function showAuthAccount(loginOrEmail = null, password = null) {
	if (loginOrEmail && password) {
		$(".authentication #auth .loginOrEmail").val(loginOrEmail);
		$(".authentication #auth .password").val(password);
		$(".authentication #auth .switch_auth").attr("checked", true);
	...
	}, 10); // Удалить позже ( Если не понимаете для чего это и почему так, не трогайте )
}
...
function regAccountHandler(emailCode) {
    ...
    if (!r.test(data.promocode)) {
		return nError("Некорректный промокод!");
    }
    ...
    if (data.login.length == 0) {
		return nError("Введите логин!");
    }
    if (data.email.length == 0) {
		return nError("Введите Email!");
    }
    if (data.password.length == 0) {
		return nError("Введите пароль!");
	}
	var password2 = data.password;
    if (password2.length == 0) {
		return nError("Повторите пароль!");
    }
    ...
    if (data.login.length < 5 || data.login.length > 20) {
		return nError("Логин должен состоять из 5-20 символов!");
    }
    if (data.email.length > 40) {
		return nError("Email должен быть менее 40 символов!");
    }
    var r = /^[0-9a-z_\.-]{5,20}$/i;
    if (!r.test(data.login)) {
		return nError("Некорректный логин!");
    }

    if (data.password.length < 6 || data.password.length > 20) {
		return nError("Пароль должен состоять из 6-20 символов!");
    }

    var r = /^[0-9a-z-_\.]+\@[0-9a-z-_]{1,}\.[a-z]{1,}$/i;
    if (!r.test(data.email)) {
		return nError("Некорректный email!");
    }
    if (password2 != data.password) {
		return nError("Пароли не совпадают!");
    }
...
function initConfirmEmailHandler() {
    var handler = () => {
        ...
        if (isNaN(code) || (code + "").length != 6) {
			return nError($("#authenticationApp .confirmEmail .code"), "Неверный код!");
        }
...
function regAccountSuccess() {
	...
	$("#authenticationApp .registration #buttonReg").attr("onclick", "");
    $("#authenticationApp .registration #buttonReg").on("click", () => {
...
function recoveryAccountHandler() {
    ...
    if (!loginOrEmail || loginOrEmail.length == 0) {
		return nError("#authenticationApp .recovery .loginOrEmail", "Заполните поле!");
    }
    ...
    if (!regLogin.test(loginOrEmail) && !regEmail.test(loginOrEmail)) {
		return nError("#authenticationApp .recovery .loginOrEmail", "Некорректное значение!");
    }
...
function authAccountHandler() {
    if (isFlood()) return;
	var loginOrEmail = $(".authentication #auth .loginOrEmail").val().trim();
    if (!loginOrEmail || loginOrEmail.length == 0) {
	...
	var password = $(".authentication .auth .password").val().trim();
    if (password.length < 6 || password.length > 20) {
