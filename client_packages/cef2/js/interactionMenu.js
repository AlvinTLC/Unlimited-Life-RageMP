$(document).ready(() => {
    ...
    window.interactionMenuAPI = {
        ...
        addItemFactionInventory: (iconName, text, itemCheck, clear) => {
          if (clear > 0) loadingItems = [];
          let haveItem = Object.keys(inventoryAPI.getArrayByItemId(parseInt(itemCheck))).length > 0;
          if (haveItem || itemCheck < 1) loadingItems.push({ icon: iconName, text: text });
        },
...
function addictivePlayerItems(data) {
    ...
    if (data.action == "showDocuments") {
        ...
        if (list[clientStorage.faction]) {
            ...
            for (let i = 0; i < loadingItems.length; i++) interactionMenuAPI.addItem(loadingItems[i].icon, loadingItems[i].text);
        }
