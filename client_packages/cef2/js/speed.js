/*$(document).ready(function () {
	window.speedController = new Vue({
		el: '.speedometer-wrapper',
		data: {
			enable: false
		},
		methods: {
			draw(speed, gear, tachometer, fuel, turnSignals) {
				// nError(`draw`);
				draw(speed, gear, tachometer, fuel, turnSignals);
				// nError(`draw ++`);
			},
	...
	const canvas = document.querySelector('.speedometer-wrapper #canvas');
	...
	// tachometer шкала тахометра от 0 до 1, где 0 это минимальное значение, а 1 макс
	// tachometer шкала бензобака от 0 до 1, где 0 это минимальное значение, а 1 макс
	// turnSignals поворотники, пример: { right: 1, left: 0 }
	// в этом случае будет включен только правый поворотник
	draw = (speedValue, gearValue, tachometerValue, fuelValue, turnSignals) => {
