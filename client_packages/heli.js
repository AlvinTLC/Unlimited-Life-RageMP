...
/** Script Keybinds **/
let keyOptions = {
    toggleCamera: 0x73,       // Keyboard: "E" Key.
...
function setHelicopterVision() {
	if(Client.vehicle.getPedInSeat(-1) != Client.handle && flirOptions.policeVisionState == 1)flirOptions.policeVisionState = 2;
...
mp.keys.bind(76, true, function() { // L
    ...
    if (Client.vehicle.model !== mp.game.joaat("polmav")) return;
    if(Client.vehicle.getPedInSeat(-1) != Client.handle) return;
    flirOptions.policeCameraLock = !flirOptions.policeCameraLock;

    mp.game.graphics.notify(flirOptions.policeCameraLock ? 'Camera locked: ~g~Enabled~w~.' : 'Camera locked: ~r~Disabled~w~.');
});
...
mp.keys.bind(0x73, true, function() { // E
    if(!Client.vehicle || chatopened) return false;
    if (Client.vehicle.model !== mp.game.joaat("polmav")) return;
	if(Client.vehicle.getPedInSeat(-1) != Client.handle) return;
    ...
    mp.game.graphics.notify(flirOptions.policeCameraEnabled ? 'Forward-looking Infrared: ~g~Enabled~w~.' : 'Forward-looking Infrared: ~r~Disabled~w~.');
});
