    ...
    if (id == "exit_menu") {
        ...
        if (id == "home") {
            ...
            if (categoryIds[id] <= 9) {
                ...
                } else mp.events.call('notify', 1, 4, "Dieser Abschnitt ist für dieses Fahrzeug nicht verfügbar.", 3000);
            }
        ...
        if (toDisable.includes(id)) {
            ...
            mp.events.call('notify', 1, 4, "Dieser Abschnitt ist für Ihr Fahrzeug nicht verfügbar.", 3000);
        }
...
mp.events.add('tclk', (id) => {
    ...
    if (setted)
        mp.events.call('notify', 1, 9, "Du hast diese Modifikation bereits installiert", 3000);
    else {
        lsc.execute(`show(${false});`);
        opened = false;
        lscSelected = id;
        ...
        var title = (lscPage === "paint_menu_one" || lscPage === "paint_menu_two" || lscPage === "paint_menu_three") ? "Sind Sie sicher, dass Sie das Auto in dieser Farbe lackieren möchten?" : "Möchten Sie diese Änderung wirklich installieren?";
    ...
    if (state) {
        if (wheelsTypes[lscPage] != undefined)
            ...
            if (paintType == 2 && isBike) {
                mp.events.call('notify', 1, 4, "Dieser Bereich ist für Motorräder nicht verfügbar.", 3000);
...
mp.events.add('tuningSeatsCheck', function () {
    ...
    for (var i = 0; i < 7; i++)
        if (veh.getPedInSeat(i) !== 0) {
            mp.events.call('notify', 4, 9, 'Bitte alle Passagiere zu gehen', 3000);
            return;
        }
