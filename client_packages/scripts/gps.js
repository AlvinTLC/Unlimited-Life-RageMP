...
mp.events.add('createGPSBlip', (x,y,z, title) => {
  ...
  mp.game.graphics.notify("~p~GPS Route für ~w~" + title + ".~p~ gesetzt. Du kannst es im Navi ausschalten.");
  gps_coords = new mp.Vector3(x,y,z);
  gps_blip = mp.blips.new(1, new mp.Vector3(x,y,z),
  {
      name: 'GPS Route: ' + title,
      scale: 1,
      color: 83,
      alpha: 255,
      shortRange: false,
...
mp.events.add('destroyGPSBlip', () => {
  if(gps_blip != null) {
    if(gps_arrived == true) {
      mp.game.graphics.notify("~p~GPS Route gelöscht.~n~~w~Du bist am Zielort angekommen.");
    }
    else {
      mp.game.graphics.notify("~p~GPS Route gelöscht!");
    }
