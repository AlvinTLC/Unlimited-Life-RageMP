...
const ClubAlcos = {
    ...
    11: ["«Auf einer Zitronenschale»", "«Auf Preiselbeeren»", "«Russischer Standard»"],
    12: ["«Asahi»", "«Midori»", "«Yamazaki»"],
    13: ["«Jivan»", "«Ararat»", "«Noyan Tapan»"],
};
...
mp.events.add("openAlco", (club, modief, isOwner, stock) => {
    ...
    alcoUI = new Menu("Club", ClubNames[club], UIPositions.LeftMiddle);
    ...
    alcoUI.AddItem(new UIMenuListItem(
        "Getränke",
        "Du kannst jedes Getränk wählen",
        new ItemsCollection(drinks)
    ));

    if (isOwner) {
        alcoUI.AddItem(new UIMenuItem("Info", `Materialien: ${stock[0]}\n${ClubAlcos[club][0]} - ${stock[1]}\n${ClubAlcos[club][1]} - ${stock[2]}\n${ClubAlcos[club][2]} - ${stock[3]}`));
        alcoUI.AddItem(new UIMenuItem("Abgefragt von", "Ausgewähltes Getränk aus dem Lager nehmen"));
        alcoUI.AddItem(new UIMenuItem("Handwerk", "Das ausgewählte Getränk zubereiten"));
        alcoUI.AddItem(new UIMenuItem("Den Preis festlegen", "Einen Preismodifikator für alle Produkte festlegen (von 50% zu 150%)"));
    }

    alcoUI.AddItem(new UIMenuItem("Kaufen", "Ein ausgewähltes Getränk kaufen"));

    var uiItem = new UIMenuItem("Schließen", "Menü schließen");
    ...
    alcoUI.ItemSelect.on(item => {
        ...
        else if (item.Text == "Abgefragt von") {
            mp.events.callRemote('menu_alco', 1, selectedAlco);
        }
        else if (item.Text == "Handwerk") {
            mp.events.callRemote('menu_alco', 2, selectedAlco);
        }
        else if (item.Text == "Den Preis festlegen") {
            global.menuClose();
            alcoUI.Close();
...
mp.events.add('stockTake', (index) => {
    ...
    switch (index) {
        case 3: //mats
            ...
            global.input.set("Die die Materialien", "Gebe die anzahl ein", 10, "take_stock");
            global.input.open();
            break;
        case 0: //cash
            mp.events.callRemote('setStock', "money");
            global.input.set("Nehme das Geld", "Geldbetrag eingeben", 10, "take_stock");
            global.input.open();
            break;
        case 1: //healkit
            mp.events.callRemote('setStock', "medkits");
            global.input.set("besorg die Erste-Hilfe-Kästen", "Gebe die Anzahl der Erste-Hilfe-Kästen ein", 10, "take_stock");
            global.input.open();
            break;
        case 2: //weed
            mp.events.callRemote('setStock', "drugs");
            global.input.set("Nehme die Medikamente", "Gebe die Menge der Medikamente ein", 10, "take_stock");
            global.input.open();
            break;
...
mp.events.add('stockPut', (index) => {
    ...
    switch (index) {
        case 3: //mats
            mp.events.callRemote('setStock', "mats");
            global.input.set("Lege die Materialien ab", "Gebe die Anzahl der Materialien ein", 10, "put_stock");
            global.input.open();
            break;
        case 0: //cash
            mp.events.callRemote('setStock', "money");
            global.input.set("Lege das Geld", "Geldbetrag eingeben", 10, "put_stock");
            global.input.open();
            break;
        case 1: //healkit
            mp.events.callRemote('setStock', "medkits");
            global.input.set("Lege die Erste-Hilfe-Kästen", "Gebe die Anzahl der Erste-Hilfe-Kästen ein", 10, "put_stock");
            global.input.open();
            break;
        case 2: //weed
            mp.events.callRemote('setStock', "drugs");
            global.input.set("Setze die Medikamente", "Gebe die Menge der Medikamente ein", 10, "put_stock");
            global.input.open();
            break;
        case 4: //weapons stock
            mp.events.callRemote('openWeaponStock');
...
function openSM(type, data) {
    ...
    switch (type) {
        ...
        case 10: menu.execute(`openFishShop('${data}');`); break; // Рыбалка //
        case 11: menu.execute(`openUSMS('${data}');`); break;
        case 12: menu.execute(`openMech('${data}');`); break;
        case 13: menu.execute(`openTuning('${data}');`); break;
        case 14: menu.execute(`openArcars('${data}');`); break;
    }
    global.menuOpen()
}
function closeSM() {
...
mp.events.add('menu', (action, data) => {
    switch (action) {
        ...
        case "fib":
            ...
            break;
        case "USMS":
            mp.events.callRemote('USMSgun', data);
            break;
        case "lspd":
        ...
        case "mafia":
            ...
            break;
		case "fishshop": // Рыбалка //
            mp.events.callRemote('fishshop', data); // Рыбалка //
            break; // Рыбалка //
        case "mech":
            mp.events.callRemote('mechitems', data);
                break;
        case "tuning":
            mp.events.callRemote('tuningManGetItem', data);
            break;
        case "arcars":
            mp.events.callRemote('arcarss', data);
           break;
    }
});
// Рыбалка //
...
mp.events.add('mechitemsUI', () => {
    let data = [
        "Funk"
    ];
    openSM(12, JSON.stringify(data)); 
});
mp.events.add('tuningManOpenItemMenu', () => {
    let data = [
        "Funk"
    ];
...
mp.events.add('fbiguns', () => {
    let data = [
        ...
        "Малый калибр x30",
        "Kleinkaliber x30",
        "Scharfschützenkaliber x5",
        "Abzeichen",
        "Funk"
    ];
    openSM(3, JSON.stringify(data));
});
mp.events.add('USMSguns', () => {
    let data = [
        "Tazer",
        "Waffe",
        "PIC",
...
mp.events.add('mavrshop', () => {
    let data = [
        ["Geldwäschedienst", ""],
        ["Eine Einbruchsübung", "100000$"],
        ["Schlossknacker", "600$"],
        ["Militärische Dietriche", "2500$"],
        ["Krawatten", "2500$"],
        ["Sack", "2500$"],
        ["Reduziere die Fahndung", "3000$"],
        ["Funk", "10000$"]
    ];
...
mp.events.add('matsL', (act) => { //load
    ...
    switch (act) {
        case 1:
            global.input.set("Materialien umladen", "Gebe die Anzahl der Materialien ein", 4, "loadmats");
            global.input.open();
            break;
        case 2:
            global.input.set("Materialien umladen", "Gebe die Anzahl der Materialien ein", 4, "loadmats");
            ...
            break;
        case 3:
            global.input.set("Medikamente umladen", "Gebe die Menge der Medikamente ein", 4, "loaddrugs");
            global.input.open();
            break;
        case 4:
            global.input.set("Erste-Hilfe-Kästen umladen", "Gebe die Anzahl der Erste-Hilfe-Kästen ein", 4, "loadmedkits");
            global.input.open();
            break;
    }
});
mp.events.add('matsU', (act) => { //unload
    ...
    switch (act) {
        case 1:
            global.input.set("Materialien umladen", "Gebe die Anzahl der Materialien ein", 4, "unloadmats");
            global.input.open();
            break;
        case 2:
            global.input.set("Materialien entladen", "Gebe die Anzahl der Materialien ein", 4, "unloadmats");
            global.input.open();
            break;
        case 3:
            global.input.set("Entlade die Medikamente", "Gebe die Menge der Medikamente ein", 4, "unloaddrugs");
            global.input.open();
            break;
        case 4:
            global.input.set("Entlade die Erste-Hilfe-Kästen", "Gebe die Anzahl der Erste-Hilfe-Kästen ein", 4, "unloadmedkits");
            global.input.open();
            break;
    }
});
...
mp.events.add('bsearch', (act) => {
    ...
    switch (act) {
        case 1:
            mp.events.callRemote('pSelected', circleEntity, "Lizenzen ansehen");
            break;
        case 2:
            mp.events.callRemote('pSelected', circleEntity, "Ansicht Reisepass");
            break;
    }
});
...
let autoColors = ["Schwarz", "Weiß", "Rot", "Orange", "Gelb", "Grün", "Blau", "Blau 2", "Lila"];
let autoModels = null;

let colors = {};
colors["Schwarz"] = [0, 0, 0];
colors["Weiß"] = [225, 225, 225];
colors["Rot"] = [230, 0, 0];
colors["Orange"] = [255, 115, 0];
colors["Gelb"] = [240, 240, 0];
colors["Grün"] = [0, 230, 0];
colors["Blau"] = [0, 205, 255];
colors["Blau2"] = [0, 0, 230];
colors["Lila"] = [190, 60, 165];
...
mp.events.add('openAuto', (models, prices) => {
    ...
    auto.entity = mp.vehicles.new(mp.game.joaat(autoModels[0]), new mp.Vector3(-42.3758, -1101.672, 26.42235),
        {
...
/*mp.keys.bind(0x78, false, function () { // F9
    ...
    if (global.menuCheck()) {
        ...
        global.menuOpen();
        donateOpened = true;
        menu.execute(`donate.show(${losts})`);
	}
});*/
...
mp.events.add('lostset', (losts_) => {
    losts = losts_;
    if (menu != null)
        menu.execute(`donate.balance=${losts}`);
});
// Help Menu //
