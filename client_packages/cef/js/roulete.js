...
$(document).ready(() => {
  ...
  $('.js-bet-submit').on('click', () => {
	if(cstate == 0) { // Поставить ставки можно только тогда, когда ставки открыты
		let lostsum = parseInt(redOutput.textContent, 10);
		let zeroSum = parseInt(zeroOutput.textContent, 10);
		let blackSum = parseInt(blackOutput.textContent, 10);
		if(totalbet + lostsum + zeroSum + blackSum <= mlimit) {
			if(lostsum >= 1 || zeroSum >= 1 || blackSum >= 1) {
				let dlostsum = parseInt(dredOutput.textContent, 10);
				...
				if(lostsum >= 1) {
					...
					if(dlostsum >= 1 && dblackSum >= 1) return;
				} else if(blackSum >= 1) {
					if(dlostsum >= 1 && dzeroSum >= 1) return;
				}
				let tlostsum = dlostsum + lostsum;
				let tzeroSum = dzeroSum + zeroSum;
				let tblackSum = dblackSum + blackSum;
				totalbet = tlostsum + tzeroSum + tblackSum;
				redOutput.textContent = 0;
				zeroOutput.textContent = 0;
				blackOutput.textContent = 0;
				dredOutput.textContent = tlostsum;
				dzeroOutput.textContent = tzeroSum;
				dblackOutput.textContent = tblackSum;
				buttonstate(0, 3);
				mp.trigger("placedbet", tlostsum, tzeroSum, tblackSum);
			}
		}
	}
  });
