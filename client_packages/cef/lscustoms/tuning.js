...
function add(obj){
    if(Array.isArray(obj[0])){
        obj.forEach((item, i, arr) => {
            $('.content_box').append(`<div id='${item[0]}' class='item'><p>${item[1]}</p><p class="p2">${item[2]}</p></div>`);
            $('#'+item[0]).hover((e) => hover(e.currentTarget.id), () => {});
            $('#'+item[0]).click((e) => click(e.currentTarget.id));
        });
    } else {
        $('.content_box').append(`<div id='${obj[0]}' class='item'><p>${obj[1]}</p><p class="p2">${obj[2]}</p></div>`)
