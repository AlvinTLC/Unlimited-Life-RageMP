...
setInterval(function () {
    var name = (localplayer.getVariable('REMOTE_ID') == undefined) ? `Noch nicht spielend` : `Spieler №${localplayer.getVariable("REMOTE_ID")}`; 
	mp.discord.update('Unlimited Life RP', name);
}, 10000);
...
mp.game.gxt.set("PM_PAUSE_HDR", 'Unlimited Life RP'); //Тут Штобы было название в меню паузы, ну типа "Тут был Вася!"
// Это отвечает за нажатие "Е" (взаимодействие) //
    mp.events.add('PressE', function (toggle) {
        pressedraw = toggle;

        if(toggle == true) 
        {
            mp.gui.execute(`HUD.pressact=('${toggle}')`);
        } 
        if (toggle == false) 
        {
            mp.gui.execute(`HUD.pressact=false;`);
        }
});
// Вот тут "Е" (взаимодействие) заканчивается //
// // // // // // //
require('./gamemode/scripts/crouch/index.js'); /*Приседание*/
require('./fingerPointer.js'); /*Палец*/
require('./gamemode/scripts/rappel/index.js'); /*Спукс с вертолета*/
require('./gamemode/scripts/house/realtor.js'); /*Риелторская контора*/
require('./gamemode/scripts/house/HouseMenu.js'); /*Меню дома*/
require('./gamemode/scripts/flatbed/flatbed.js'); /*Пиздить машины на эвакуаторе, акуенное занятие*/
require('./changeplate/index.js'); //Смена номеров для машин
require('./bizinfo.js'); //Биз_Инфо (менюшка) от Найтвилл
...
require('./main.js');
require('./voice.js');
require('./sonstiges/sirene.js');
//require('./heli.js');
require('funk/index.js');
//require('deathscreen');
//require('./autoshow.js')
require('./house/realtor.js');
//require('./jobsGolem/farmer/');
//require('./jobsGolem/market/');
...
///
require ('./ped/infoped.js');

require('./rage-rpc.min.js');
//require('./animationswechsler/index.js')

require("./heistisland/heistisland.js");
//require('./anticheat/index')
require('./newWork/work');
require('./newWork/colshapes');
require('./newWork/peds');
...
//require('./weapondisplay.js'); //weapondisplay
require('./gamemode/scripts/ts/index1.js');
require('./gamemode/scripts/forrest.js');
require('./gamemode/scripts/sprunk.js');
//require('./cef2/ipad.js');

//fractions=====================
require('./fractions/vehiclespawner.js');
//==============================
...
mp.events.add('takeOffWeapon', (weaponHash) => {
    try {
		...
		givenWeapon = -1569615261; /*HASH пустой руки*/
...
mp.game.ped.setAiMeleeWeaponDamageModifier(0.2);
...
const arcadeId = mp.game.interior.getInteriorAtCoordsWithType(
  2730.0,
  -380.0,
  -49.0,
  "ch_DLC_Arcade"
);
const planRoomId = mp.game.interior.getInteriorAtCoordsWithType(
  2730.0,
  -380.0,
  -49.0,
  "ch_DLC_Plan"
...
for (const [group, props] of Object.entries(propList)) {
  for (const prop of props) {
    mp.game.interior.enableInteriorProp(
      group === "arcade" ? arcadeId : planRoomId,
      prop
    );
  }
}
