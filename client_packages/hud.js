...
var showHint = true;
var lastButBet = 0;
global.isjoinTable = false;
global.statusGame = false;
global.ReadyGame = false;
...
// HUD events
mp.events.add('updatelastbonus', function (bonus) {
...
mp.events.add('showHUD', (show) => {
    ...
    if(show) mp.gui.execute(`HUD.server=${serverid};`);
...
}); Это гавно отвечает за мигание карты и звездочки в углу - розыск*/
...
mp.keys.bind(Keys.VK_6, false, function () { // 5 key - cruise mode on/off
    ...
    if (new Date().getTime() - cruiseLastPressed < 300) {
        mp.events.call('openInput', 'Tempomat', 'Gebe die Geschwindigkeit in km/h an', 3, 'setCruise');
    } else {
...
mp.events.add('setCruiseSpeed', function (speed) {
	...
	if(speed < curSpeed) {
		mp.events.call('notify', 4, 9, "Du kannst die Geschwindigkeit nicht niedriger einstellen als sie momentan ist, reduziere die Geschwindigkeit und versuche es erneut.", 6000);
		return;
	}
...
mp.events.add('Playerlisttable', function (json) {
    mp.gui.execute(`diamond.playername='${mp.players.local.name}';`);
    mp.gui.execute(`diamond.setC('${json}');`);

});
mp.events.add('rolling', function () {
    mp.events.callRemote('roll');
});
mp.events.add('toready', function (ready) {
    ReadyGame = ready;
});
mp.events.add('setstatusdiamond', function (statusGa,time) {
    mp.gui.execute(`diamond.statusGa=${statusGa}`);
    statusGame = statusGa;
    if(statusGame) {
        mp.gui.execute(`diamond.timer=${time}`);
        mp.gui.execute(`diamond.countdown()`);
    }

});
mp.events.add('joinTotable', function (tabelBet,statusGa) {

        global.isjoinTable = true;
        mp.gui.execute(`diamond.showList=true`);
    mp.gui.execute(`diamond.bet='${tabelBet}'`);
    mp.gui.execute(`diamond.statusGa=${statusGa}`);
    localplayer.freezePosition(true);
    menuOpen();

});
mp.events.add('tobet', function (tobet) {
    if (new Date().getTime() - lastButBet < 15000) {
        mp.events.call('notify', 4, 9, "Zu schnell", 2000);
        return;
    }
    lastButBet = new Date().getTime();
    mp.events.callRemote('tobet',tobet);
});
mp.events.add('leavetable', function () {
    menuClose();
    global.isjoinTable = false;
    statusGame=false;
    ReadyGame=false;
    mp.gui.execute(`diamond.showList=false`);
    ...
    players.forEach((id) => {
        ...
        if (mp.players.exists(player)) {
			...
			if(player.getVariable('IS_MASK') == true) {
				name = (player === localplayer || localplayer.getVariable('IS_ADMIN') == true) ? `${player.name.replace("_", " ")} (${player.getVariable('REMOTE_ID')})` : `Игрок (${id})`;
			} else {
...
mp.events.add('render', (nametags) => {
    ...
    if(hudstatus.id != localplayer.getVariable("REMOTE_ID"))
    {
        hudstatus.id = localplayer.getVariable("REMOTE_ID"); 
        mp.gui.execute(`HUD.id='${hudstatus.id}'`);
    }
    ...
    if (localplayer.isInAnyVehicle(false)) {
		...
		if(localplayer.vehicle.getPedInSeat(-1) == localplayer.handle) {
			...
			if (veh.getVariable('FUELTANK') !== undefined) {
				...
				mp.game.graphics.drawText(`Heruntergeladen von: ${fueltank}/1000L`, [0.93, 0.80], {
					font: 0,
			...
			let rpm = (veh.rpm * 10000).toFixed();
			if (rpm !== hudstatus.rpm) {
